/**
 * Unit tests for amv-context-injector extension.
 *
...
/**
 * Create a minimal AMV vault structure in tmpDir.
...
describe("findVaultRoot", () => {
	...
	test("finds vault root from subdirectory", () => {
		...
		const subdir = path.join(tmpDir, "skills", "agent-memory-vault");
...
describe("buildSessionHandoff", () => {
	...
	test("includes related link to manifest", () => {
		const handoff = buildSessionHandoff("s1", "autra", "Test");

		expect(handoff).toContain("[[packs/autra/00-system/pi-agent/memory-manifest|Memory Manifest]]");
	});
});
...
describe("extension registration", () => {
	...
	test("registers amv_search tool", () => {
		const { pi, tools } = createMockPi();
		registerExtension(pi as any);

		expect(tools["amv_search"]).toBeDefined();
		expect(tools["amv_search"].name).toBe("amv_search");
		expect(tools["amv_search"].description).toContain("Search across Agent Memory Vault");
	});
...
// ==========================================================================
// 10. amv_search tool execution (grep fallback)
// ==========================================================================

describe("amv_search tool (grep fallback)", () => {
	beforeEach(() => {
		setupTmpDir();
	...
	test("returns error when no active pack", async () => {
		...
		const result = await tools["amv_search"].execute("call-1", { query: "test" }, null, () => {}, {});
	...
	test("finds matching files by keyword", async () => {
		...
		const result = await tools["amv_search"].execute(
			"call-1",
			{ query: "PostgreSQL" },
			null,
			() => {},
...
describe("before_agent_start context injection", () => {
	...
	test("injects pack context into system prompt", async () => {
		...
		expect(result.systemPrompt).toContain("Agent Memory Vault");
		expect(result.systemPrompt).toContain("test-pack");
		expect(result.systemPrompt).toContain("TypeScript");
	});

	test("includes amv_search tool hint in injection", async () => {
		const { packPath } = createTestVault();
		const { pi, hooks } = createMockPi();
		registerExtension(pi as any);
		_setActivePack("test-pack", packPath);
		...
		expect(result.systemPrompt).toContain("amv_search");
	});
});
...
describe("integration: full flow", () => {
	...
	test("search → inject → compact cycle", async () => {
		...
		// 1. Search
		const searchResult = await tools["amv_search"].execute(
			"c1",
		...
		const searchAfter = await tools["amv_search"].execute(
			"c2",
